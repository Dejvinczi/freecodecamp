...
export default function AssemblyEndgame() {
  ...
  // Derived values
  const wrongGuessCount = guessedLetters.filter((letter) => !currentWord.includes(letter)).length || 0;

  const isGameLost = wrongGuessCount === languages.length;
  const isGameWon = currentWord.split("").every((c) => guessedLetters.includes(c));
